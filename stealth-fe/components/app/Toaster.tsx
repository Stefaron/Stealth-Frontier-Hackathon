...
function Toast({ item, onDismiss }: { item: ToastItem; onDismiss: () => void }) {
  ...
  return (
    <div className="relative flex items-start gap-3 bg-[#1a1917] border border-white/[0.08] rounded-2xl px-4 py-3.5 shadow-xl min-w-[280px] max-w-[380px] overflow-hidden">
      <div className={`absolute left-0 top-0 bottom-0 w-0.5 rounded-l-2xl ${config.bar}`} />
      <span className={`flex-shrink-0 mt-0.5 ${config.iconColor}`}>{config.icon}</span>
      <p className="flex-1 text-white/80 text-[12px] leading-snug pr-4">{item.message}</p>
      <button
        onClick={onDismiss}
